...
exports.sfdcAuthenticate = async (context, worker) => {
  ...
  try {
    ...
    if (
      ...
      const newTokens = await accessToken(context, salesforceUsername);
      const connection = new jsforce.Connection({
...
const accessToken = async (context, salesforceUsername) => {
  const threeMinutesFromNowInSeconds = Math.floor(Date.now() / 1000) + 3 * 60;
  let params = {
    clientId: context.SF_CLIENT_ID, 
    clientSecret: context.SF_CLIENT_SECRET,
    loginUrl: context.SFDC_INSTANCE_URL,
  };
  try {
    const oauth = new jsforce.OAuth2(params);
    let token = await oauth.authenticate(
        context.SF_USERNAME, 
        context.SF_PASSWORD,
    );
